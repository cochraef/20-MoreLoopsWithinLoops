...
def run_test_largest_number():
    ...
    # Test 4:
    expected = 11
    answer = largest_number([[1, 2, 3], [1, 2, 4], [1, 2, 3, 6, 10], [2, 3, 5, 7, 11]])
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    largest = -math.inf
    for sub_seq in seq_seq:
        for number in sub_seq:
            if number > largest:
                largest = number
...
def run_test_largest_negative_number():
    ...
    # Test 1:
    expected = -0.01
    answer = largest_negative_number([[-1, -0.1, -0.01], [-2, -3, -4]])
    print('Expected and actual are:', expected, answer)
    ...
    # Test 4:
    expected = None
    answer = largest_negative_number([[1, 2, 3], [1, 2, 4], [1, 2, 3, 6, 10], [2, 3, 5, 7, 11]])
    print('Expected and actual are:', expected, answer)
...
def first_is_elsewhere_too(seq_seq):
    ...
    first_sub_seq = seq_seq[0]

    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            for i in range(len(first_sub_seq)):
                if seq_seq[k][j] == first_sub_seq[i] and k != 0:
                    return True
